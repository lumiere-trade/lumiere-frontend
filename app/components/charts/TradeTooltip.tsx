      ...
      <div className="bg-background border-2 border-primary/20 rounded-lg shadow-xl p-4 w-80 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 mb-3 pb-2 border-b border-border">
          ...
          <div className="flex-1">
            <div className={`inline-block px-2.5 py-1 rounded text-white text-base font-semibold ${tradeType.color}`}>
              {tradeType.label}
            </div>
          </div>
        ...
        {/* Reason */}
        <div className="mb-3">
          <div className="text-base font-semibold text-muted-foreground mb-1.5">Reason:</div>
          {hasMultipleConditions ? (
            ...
            <p className="text-base text-foreground font-medium">{trade.reason || 'Signal'}</p>
          )}
        </div>
          ...
          <div className="mb-3 pb-3 border-b border-border">
            <div className="grid grid-cols-2 gap-3 text-base">
              <div>
                ...
                <div className="font-mono font-semibold text-base">{trade.q.toFixed(4)}</div>
              </div>
              <div>
                <div className="text-muted-foreground font-medium mb-0.5">
                  {trade.s === 'B' ? 'Invested' : 'Exit Value'}
                </div>
                <div className="font-mono font-semibold text-base">${trade.val.toFixed(2)}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 mb-3 text-base">
          <div>
            ...
            <div className="font-mono font-semibold text-base">${trade.p.toFixed(2)}</div>
          </div>
          {trade.pnl !== undefined && trade.pnl !== null && (
            <div>
              <div className="text-muted-foreground font-medium mb-0.5">P&L</div>
              <div className={`font-mono font-semibold text-base ${trade.pnl >= 0 ? 'text-green-500' : 'text-red-500'}`}>
        ...
        {trade.indicators && Object.keys(trade.indicators).length > 0 && (
          <div className="border-t border-border pt-3">
            <div className="text-base font-semibold text-muted-foreground mb-2">Key Indicators:</div>
            <div className="grid grid-cols-2 gap-2.5">
              {Object.entries(trade.indicators).slice(0, 4).map(([name, value]) => (
                <div key={name} className="text-base">
                  ...
                  <div className="font-mono font-semibold text-base">{value.toFixed(2)}</div>
                </div>
              ))}
            </div>
