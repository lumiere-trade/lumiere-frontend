...
import { PnLChartRenderer } from './pnlChartRenderer'
import { calculatePnLViewport } from './pnlChartUtils'
import type { PnLPoint, PnLState } from './types'

interface PnLChartProps {
  data: Array<{
    timestamp: number
    pnl: number  // cumulative PnL in dollars
  }>
  height?: number
}
...
export const PnLChart = memo(function PnLChart({
  ...
  const rendererRef = useRef<PnLChartRenderer | null>(null)
  ...
  const [state, setState] = useState<PnLState>({
    points: [],
    viewport: { startIdx: 0, endIdx: 0, pnlMin: 0, pnlMax: 0, zoom: 1, offsetX: 0, width: 0 },
    mouse: null,
    isDragging: false,
    dirty: true
  })
  ...
  useEffect(() => {
    ...
    rendererRef.current = new PnLChartRenderer(canvasRef.current)
  }, [])
  ...
  useEffect(() => {
    ...
    const viewport = calculatePnLViewport(points, zoom, offsetX, containerWidth)
    setState(s => ({
      ...s,
      points,
      viewport,
